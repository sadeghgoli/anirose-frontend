...
const Addresses = () => {
  ...
  const [form, setForm] = useState({
    first_name: "", last_name: "", province_id: "", city_name: "",
    full_address: "", postal_code: "", mobile: "", is_default: false,
  });
  ...
  const resetForm = () => {
    setForm({ first_name: "", last_name: "", province_id: "", city_name: "", full_address: "", postal_code: "", mobile: "", is_default: false });
    setEditingId(null);
    setShowForm(false);
  };
  ...
  const handleEdit = (addr) => {
    setForm({
      ...
      province_id: addr.province_id || "", city_name: addr.city_name || "",
      full_address: addr.full_address || addr.address || "", postal_code: addr.postal_code || "",
      mobile: addr.mobile || "", is_default: addr.is_default || false,
    });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setSaving(true);
    const selectedProvince = provinces.find((p) => p.id === Number(form.province_id));
    const payload = {
      ...form,
            ...
            <div className="bg-[#F4F7F5] rounded-2xl p-6 mb-6 border border-[#0c5505]/10">
              ...
              <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    <label className="block mb-1.5 text-sm font-medium text-gray-700">استان</label>
                    <select value={form.province_id} onChange={(e) => setForm({ ...form, province_id: e.target.value })} className={inputClass} required>
                  ...
                  <div>
                    ...
                    <input id="address-city" type="text" value={form.city_name} onChange={(e) => setForm({ ...form, city_name: e.target.value })} className={inputClass} required />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block mb-1.5 text-sm font-medium text-gray-700">آدرس کامل</label>
                    <textarea value={form.full_address} onChange={(e) => setForm({ ...form, full_address: e.target.value })} className={inputClass} rows="2" required />
                  </div>
                  <div>
